...
create schema f_coches;

-- Creando las tablas que no tendran claves foraneas

create table f_coches.grupos (
	id smallserial primary key,
	nombre_grupo varchar(100)
);

create table f_coches.colores (
	id smallserial primary key,
	nombre_color varchar(40)
);

create table f_coches.aseguradoras (
	id smallserial primary key,
	nombre_aseguradora varchar(30)
);

create table f_coches.monedas (
	id smallserial primary key,
	nombre_moneda varchar(50)
);


-- Luego la creacion de las tablas que contienen claves foraneas

create table f_coches.marcas (
	id smallserial primary key,
	nombre_marca varchar(20),
	id_grupo smallint references f_coches.grupos(id)
);

create table f_coches.modelos (
	id smallserial primary key,
	nombre_modelo varchar(20),
	id_marca smallint references f_coches.marcas(id)
);

create table f_coches.coches (
	id smallserial primary key,
	fecha_compra date,
	km_totales integer,
	id_marca smallint references f_coches.marcas(id),
	id_color smallint references f_coches.colores(id),
	id_modelo smallint references f_coches.modelos(id)
);

create table f_coches.matriculas (
	matricula varchar(10) primary key,
	id_coche smallint references f_coches.coches(id)
);

create table f_coches.revisiones (
	id smallserial primary key,
	fecha_revision date,
	km_revision integer,
	importe_revision float4,
	id_coche smallint references f_coches.coches(id)
);

create table f_coches.polizas (
	n_poliza integer,
	fecha_alta_seguro date,
	id_coche smallint references f_coches.coches(id),
	id_aseguradora smallint references f_coches.aseguradoras(id),
	id_moneda smallint references f_coches.monedas(id),
	primary key (n_poliza, id_moneda)
);


-- IMPORTANDO DATOS PARA LLENAR LA TABLA IMPORT_COCHES

CREATE TABLE f_coches.import_coches (
...
INSERT INTO f_coches.import_coches (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- INSERTANDO LOS DATOS EN LAS TABLAS DEL ESQUEMA f_coches
	
-- Poblando la tabla grupos
	
insert into f_coches.grupos (nombre_grupo) 
select distinct grupo 
from f_coches.import_coches;
	  
-- Ploblando la tabla colores
insert into f_coches.colores (nombre_color)
select distinct color 
from f_coches.import_coches;

-- Ploblando la tabla monedas
insert into f_coches.monedas (nombre_moneda)
select distinct moneda 
from f_coches.import_coches;

-- Ploblando la tabla aseguradoras
insert into f_coches.aseguradoras (nombre_aseguradora)
select distinct aseguradora from f_coches.import_coches;


-- Poblando la tabla marcas
insert into f_coches.marcas (nombre_marca, id_grupo)
select distinct ic.marca, gp.id  
from f_coches.import_coches ic
inner join f_coches.grupos gp
on ic.grupo  = gp.nombre_grupo;

-- Poblando la tabla modelos
insert into f_coches.modelos (nombre_modelo, id_marca)
select distinct ic.modelo, m.id 
from f_coches.import_coches ic
inner join f_coches.marcas m 
on ic.marca = m.nombre_marca;



-- Poblando la tabla coches
insert into f_coches.coches (fecha_compra, km_totales,id_marca, id_color, id_modelo)
select distinct ic.fecha_compra, ic.kms_totales, m.id, c.id, m2.id 
from f_coches.import_coches ic
inner join f_coches.marcas m 
on ic.marca = m.nombre_marca 
inner join f_coches.colores c 
on ic.color = c.nombre_color
inner join f_coches.modelos m2
on ic.modelo = m2.nombre_modelo 
order by ic.fecha_compra 
;

-- Poblando la tabla matriculas
insert into f_coches.matriculas (matricula, id_coche)
select distinct ic.matricula, c.id from f_coches.import_coches ic
inner join f_coches.coches c
on ic.kms_totales = c.km_totales
order by c.id
;


-- Poblando la tabla revisiones
insert into f_coches.revisiones (fecha_revision, km_revision, importe_revision, id_coche)
select ic.fecha_revision, ic.kms_revision, ic.importe_revision, c.id
from f_coches.import_coches ic
inner join f_coches.coches c 
on ic.kms_totales = c.km_totales
order by ic.fecha_revision ;

-- Poblando la tabla polizas
insert into f_coches.polizas (n_poliza, fecha_alta_seguro, id_coche, id_aseguradora, id_moneda)
select distinct ic.n_poliza, ic.fecha_alta_seguro, c.id, a.id, m.id
from f_coches.import_coches ic
inner join f_coches.coches c 
on ic.kms_totales = c.km_totales
inner join f_coches.aseguradoras a 
on ic.aseguradora = a.nombre_aseguradora 
inner join f_coches.monedas m 
on ic.moneda = m.nombre_moneda 
order by c.id;

-- SELECTS


select c.id, m2.nombre_modelo, m.nombre_marca, g.nombre_grupo, c.fecha_compra,
	   m3.matricula, c2.nombre_color, c.km_totales,a2.nombre_aseguradora, p.n_poliza 
from f_coches.grupos g
inner join f_coches.marcas m 
on g.id = m.id_grupo
inner join f_coches.modelos m2 
on m.id = m2.id_marca 
inner join f_coches.coches c 
on m2.id = c.id_modelo
inner join f_coches.matriculas m3
on m3.id_coche = c.id
inner join f_coches.colores c2 
on c2.id = c.id_color
inner join f_coches.polizas p 
on p.id_coche = c.id
inner join f_coches.aseguradoras a2 
on a2.id = p.id_aseguradora
order by c.id
;
